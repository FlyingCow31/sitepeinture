...
export function ServiceDiv({ imgSrc, title, text, side, side2 = "left" }: ServiceDiv) {
     return (
          <article className={`flex lg:flex-row ${side === "right" ? "lg:flex-row-reverse" : ""} justify-around mt-16`}>
               <div className="relative">
                    ...
                    {side2 != null && (
                         <div
                              className={`h-120 w-120 bg-transparent border-sec absolute ${side2 == "right" ? "-left-10 border z-11" : "left-10 border-3"} top-10 `}
                         ></div>
                    )}
               </div>

               <div className={`flex flex-col ${side === "right" ? "text-left" : "text-right"}`}>
...
export function ServiceSpecial({ children, className = "" }: ServiceSpecial) {
     return <article className={`flex lg:flex-row-reverse justify-around mt-16 ${className}`}>{children}</article>
}
